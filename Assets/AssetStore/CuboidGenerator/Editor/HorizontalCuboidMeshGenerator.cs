...
namespace GeneratedCuboids
{
    public class HorizontalCuboidMeshGenerator : AbstractCuboidMeshGenerator
    {
        ...
        {
            tempVec = new Vector2(y * uvScale, y * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2((x + y) * uvScale, y * uvScale);
            uvCoordinates += AddToUVs();
            ...
            uvCoordinates += AddToUVs();
            tempVec = new Vector2((2 * y + 2 * x) * uvScale, y * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2((2 * y + 2 * x) * uvScale, (y + z) * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2(y * uvScale, (2 * y + z) * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2((x + y) * uvScale, (2 * y + z) * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2(0, y * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2(0, (y + z) * uvScale);
            uvCoordinates += AddToUVs();
            tempVec = new Vector2((x + 2 * y) * uvScale, y * uvScale);
            uvCoordinates += AddToUVs();
